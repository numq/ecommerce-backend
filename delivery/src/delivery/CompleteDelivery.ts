...
@injectable()
export class CompleteDelivery extends UseCase<string, Delivery> {
    ...
    execute = (arg: string): TaskEither<Error, Delivery> => pipe(
        ...
        TE.chain(delivery => this.repository.updateDelivery({...delivery, status: DeliveryStatus.Completed})),
        TE.chain(TE.fromNullable(DeliveryError.NotFound)),
